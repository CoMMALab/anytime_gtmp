...
class TensorState:
    ...
    splines: Any = struct.field(default=None, pytree_node=False)
...
def gtmp_plan(key: jax.Array, state: TensorState, spline_interpolator: Optional[Any] = None) ->  TensorOutput:
        q = state.q
        ...
        if spline_interpolator:
            splines = spline_interpolator(state.time_profile, q, dream_points, state.goals)
        ...
        scale_occ, scale_dist = state.scale_occ, state.scale_dist
        ...
        if state.num_layers > 1:
            ...
            Vs, Vh = value_iteration_fn(Cs, Ch, Cl, Cg, **({'gamma': gamma} if not state.vi_finite else {}), dtype=state.dtype)
        else:
            Cs = jnp.squeeze(Cs)
        ...
        if spline_interpolator:
            def get_path_akima(_):
                if state.num_layers > 1:
                    mid_idx, goal_idx =  get_optimal_path(Cs, Ch, Cl, Cg, Vh, gamma)
                    path_ids = jnp.append(jnp.append(0, mid_idx), goal_idx)
                else:
            ...
            path, path_ids, goal_idx = lax.cond(collision, 
                lambda _: (jnp.zeros((state.num_probes * (state.num_layers + 1), q.shape[-1]), state.dtype), jnp.zeros(state.num_layers + 2, dtype=jnp.int32), 0), 
                get_path_akima, None)
            
            if state.get_velocity and not collision:
                spline_vel = splines.derivative()
                path_vel = spline_vel.get_spline_interpolation(path_ids, num_points=state.num_probes)
        else:
            def get_path_linear(_):
                if state.num_layers > 1:
                    mid_idx, goal_idx =  get_optimal_path(Cs, Ch, Cl, Cg, Vh, gamma)
            ...
            path, goal_idx = lax.cond(collision, 
                lambda _: (jnp.zeros((state.num_layers + 2, q.shape[-1]), state.dtype), 0), 
                get_path_linear, None)

        output =  TensorOutput(path=path, goal_idx=goal_idx, collision=collision, path_vel=path_vel, splines=splines)
