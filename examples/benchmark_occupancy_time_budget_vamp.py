...
def save_paths_gif(paths, costs, npy_path, output_path, title="", fps=4, max_paths=None):
    """
    Animate one path per iteration. The running global-best (lowest cost so far)
    is drawn in red; all others in blue. Straight-line GTMP waypoints.

    paths: list of (N, 3) arrays in world [x, y, z] coordinates, in iteration order.
    ...
    for n in range(1, len(display_paths) + 1):
        ...
        for i, path in enumerate(display_paths[:n]):
            ...
            ax.plot(pts[:, 0], pts[:, 1], '-o',
                    color='tab:blue', linewidth=1.0, markersize=2, alpha=0.45)
        best_pts = np.asarray(display_paths[best_i])
        ax.plot(best_pts[:, 0], best_pts[:, 1], '-o',
                color='red', linewidth=1.8, markersize=3, alpha=0.95, zorder=5)
...
def main():
    ...
    if len(iter_paths) > 0:
        ...
        print(f"First path has {len(first)} waypoints")
        print(f"  Start: {first[0]}")
        print(f"  End:   {first[-1]}")

        vis_iter_paths = iter_paths[:100]
        raw_paths = [np.asarray(entry['path']) for entry in vis_iter_paths]
        ...
        print(f"Shortcutting {len(raw_paths)} per-iteration best paths for visualization...")
        batched = np.stack(raw_paths, axis=0)
        simplified_batched, _ = simplify_paths_batched(
            batched,
            np.ones(len(raw_paths), dtype=bool),
            edge_validator=_edge_validator,
            num_shortcut_rounds=64,
            seed=0,
        )
        paths = [simplified_batched[i] for i in range(len(raw_paths))]
        costs = [
            float(np.sum(np.linalg.norm(np.diff(p, axis=0), axis=-1)))
            for p in paths
        ]

        out = get_gallery_path() / "benchmark_occupancy_time_budget_vamp.gif"
        save_paths_gif(paths, costs, npy_path, out, title="GTMP + VAMP (anytime)")
    else:
