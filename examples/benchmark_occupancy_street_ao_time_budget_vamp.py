...
def save_ao_progress_gif(
    iter_best_paths, img_array, output_path, q_3d, goal_3d,
    ...
    for i, entry in enumerate(entries):
        ...
        # correspond to vamp's obstacles.
        ax.imshow(
            np.fliplr(img_array),
...
def main():
    ...
    if len(iter_best_paths) > 0:
        ...
        out = (
            get_gallery_path()
            / f"benchmark_occupancy_street_ao_time_budget_vamp_{png_stem}_pair{args.pair_index}.gif"
        )
        save_ao_progress_gif(
            iter_best_paths, img_array, out,
            q_3d=start_np, goal_3d=goal_np,
            world=WORLD_SIZE,
            title=f"AO-GTMP + VAMP — {args.png_name}",
        )
    else:
